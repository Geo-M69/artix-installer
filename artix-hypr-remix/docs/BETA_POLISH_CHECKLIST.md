...
| Wallpaper/theme handling | present | `ahr-theme*`, `default/themes/*`, `default/themed/*.tpl`, `ahr-launch-wallpaper-session`, `ahr-theme-bg-gallery`, `ahr-theme-bg-switcher` | `../omarchy/themes/*`, `omarchy-theme-*`, `omarchy-theme-bg-*` | High-impact polish | Theme set/list/current/refresh/background state all present. Gallery preview (fzf+chafa/imv) added 2026-06-09; Walker plugin (`ahrBackgroundSelector.lua`) deployed and discoverable. |
...
- [x] Background selection has a picker/gallery path — fzf+chafa terminal gallery (`ahr-theme-bg-gallery`), imv GUI gallery fallback, Walker plugin (`ahrBackgroundSelector.lua`) with image previews, and the classic dmenu/TTY list picker (`ahr-theme-bg-switcher`).
