...
| OpenRC services | present | `services/openrc-default.txt`, `lib/openrc.sh`, `scripts/post-install-smoke.sh` | Omarchy systemd service usage | Public beta blocker | Required core services are `dbus`, `elogind`, and `NetworkManager`; `bluetoothd` is optional. The printing profile separately requires `cupsd` and `avahi-daemon` only when selected. |
...
| Default apps | implemented | `first-run.d/56-default-apps.sh`, `ahr-default-browser`, `ahr-default-terminal`, `ahr-default-calculator`, `ahr-default-editor`, `ahr-launch-*` | Omarchy Defaults menu | High-impact polish | Browser, terminal, file manager, editor, calculator, PDF (Evince), image (imv), video/audio (mpv), and archive (File Roller) defaults are implemented and data-driven via `ahr-default-apps-matrix.sh`; idempotent and choice-preserving. |
...
| Document Viewer / PDF viewer | Implemented (Evince) | Required for final | PDF default app and form/signing guidance | Evince is the default PDF viewer (native package, `application/pdf` → `org.gnome.Evince.desktop`); added in Phase 2. |
...
| mpv | Implemented (native) | High-value polish | Media player package/default | `mpv` is the default for common video AND audio MIME types (Phase 2); audio MIME explicitly maps to `mpv.desktop`. |
