...
- `scripts/post-install-smoke.sh` validates required core OpenRC services (`dbus`, `elogind`, `NetworkManager`), while `bluetoothd` is reported as optional. Baseline validation does not require printing services; use `--expect-printing on` only when the printing profile was selected, which requires `cupsd` and `avahi-daemon`. `--expect-printing auto` (the default) does not infer profile selection from installed packages or init scripts. It also validates desktop runtime command dependencies (`polkit-gnome-authentication-agent-1`, `xdg-desktop-portal`, `xdg-desktop-portal-hyprland`), active session runtime processes (`xdg-desktop-portal`, `xdg-desktop-portal-hyprland`, `pipewire`, `wireplumber`) when a Hyprland session is running, Hyprland runtime command dependencies (with optional warnings for `walker`/`elephant`), wallpaper backend availability (`swww` or `swaybg`), startup mode state, session launcher executable state, and mode-specific startup wiring.
...
- Milestone 6 beta checklist: [MILESTONE6_BETA_CHECKLIST.md](MILESTONE6_BETA_CHECKLIST.md)
- Known issues and troubleshooting: [MILESTONE6_KNOWN_ISSUES.md](MILESTONE6_KNOWN_ISSUES.md)
- Beta release notes (draft): [RELEASE_NOTES.md](RELEASE_NOTES.md)
- Screenshots / expected-result artifacts: [docs/screenshots/](docs/screenshots/)
