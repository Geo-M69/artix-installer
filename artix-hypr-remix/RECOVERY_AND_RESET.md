...
- Detect core managed config presence with `ahr repair --config`.

## Detect-Only Areas

These areas are intentionally not rewritten automatically yet:
...
For now, config recovery that rewrites files should use installer phase 4, which creates timestamped backups before replacing managed config roots.
...
1. Run `ahr repair` as the desktop user and review the `FIX:` lines.
2. Run scoped repair commands first, such as `ahr repair --namespace --apply` or `ahr repair --theme --apply`.
3. Run `ahr migrate --status` and `ahr migrate --dry-run`.
4. Retry skipped migrations only after reviewing `migrate.log`.
5. Use installer phase 4 only when source-deployed config files are missing or badly drifted.
6. Run `./scripts/post-install-smoke.sh --user <username>` after repair on a real host.
